...
public class SlackService {
  ...
  public String getRecentMessages(String channelName) {
    ...
    if (channelId == null) {
      ...
      log.warn(
          "Channel '{}' not found in config, falling back to '{}'", channelName, fallback.getKey());
      prefix =
          (channelName != null ? "I couldn't find #" + channelName + ", but " : "")
    ...
    try (Slack slack = Slack.getInstance()) {
      ...
      ConversationsHistoryResponse response =
          methods.conversationsHistory(
              r -> r.channel(resolvedChannelId).limit(slackConfig.getMessageLimit()));

      if (!response.isOk()) {
        log.warn(
            "conversations.history failed for #{}: {}", resolvedChannelName, response.getError());
