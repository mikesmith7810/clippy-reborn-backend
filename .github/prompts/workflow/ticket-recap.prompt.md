---
description: Generate a visual HTML ticket recap — rebuild mental model of a ticket's current state, recent decisions, and cognitive debt hotspots
---
Load the visual-explainer skill, then generate a comprehensive visual ticket recap as a self-contained HTML page.
...
**Data gathering phase** — run these first to understand the ticket. The ticket information is contained in the `tickets/{ticket}/` directory:

1. **Ticket identity.** Read `plain.txt`, `raw.json` for name, description, dependencies. Read the top-level file structure.

2. **Solution activity.** Read `research.md` for context. Read `solution.md` for the proposed solution. Read `plan_review_facts.md` for any plan review facts.

3. **Current state.** Read `todo.md` for open tasks and progress notes. Read the `diff_review_facts.md` for any recent code changes related to the ticket. Look for TODO/FIXME comments in recently changed files.

4. **Decision context.** Read recent commit messages for rationale. If running in the same session as recent work, mine the conversation history. Read any plan docs, RFCs, or ADRs in the ticket directory.
...
**Optional hero image** — if `surf` CLI is available (`which surf`), generate a hero banner via `surf gemini --generate-image --aspect-ratio 16:9` that visually captures the ticket's identity or domain. Match the style to the page's palette. Embed as base64 data URI using the `.hero-img-wrap` pattern from css-patterns.md. Place above or just below the title. Skip if surf isn't available — the page should stand on its own.

**Diagram structure** — the page should include:
1. **ticket identity** — not the README blurb. A *current-state* summary: what this ticket does, who uses it, what stage it's at (early dev, stable, actively shipping features). Include version, key dependencies, and the one-sentence "elevator pitch" for someone who forgot what they were building.
...
6. **Mental model essentials** — the 5-10 things you need to hold in your head to work on this ticket effectively:
...
8. **Next steps** — inferred from recent activity, open TODOs, ticket trajectory. Not prescriptive — just "here's where the momentum was pointing when you left." Include any explicit next-step notes from progress docs or plan files.
